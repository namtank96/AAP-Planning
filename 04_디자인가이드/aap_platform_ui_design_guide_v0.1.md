...
> **작성 기준**: 토큰·기본 패턴은 v0.22 데모 `:root`에서 *역추출*. **단 현행 라이브 기준선은 `05_범용플랫폼/maestro/`(AAP Maestro 운영 콘솔 — 옛 `app/`, v0.22 정본 승격)** — §6.5 이후 절(운영 콘솔 IA·온톨로지·블록·3패널 빌더·운영 콘솔·레이아웃·케이스 튜너)은 모두 그 구현 역추출이다. 새 색 발명 ✕ — 토큰만.
...
6. **메인은 작게, 무게는 모달 — 단 HITL '결정 요약'은 본문 인라인(v0.9 정정).** 메인 화면은 한눈에. **사람 결정이 필요한 지점은 흐름을 끊는 덮는 모달이 아니라 본문 인라인 결정 카드**(개입 이유·AAP 판단·근거·추천·액션)로 보인다. 실제 **기준 조정(슬라이더·비교표)·상세 근거·긴 리포트·산출물**만 모달(on-demand). → §5.6.
...
*(`05_범용플랫폼/maestro/` 구현 = 표준. 이모지 톤 배제 §1 Notion 차용 원칙과 정합.)*

- **표준 = Lucide** (stroke 기반, 미니멀). 이모지(📥▶🛡️🧩🧠📅✓ 등) → Lucide SVG로 교체.
- **외부 라이브러리 0 · `file://` 더블클릭 동작** 제약 → CDN/npm 금지, **인라인 SVG 임베드**. 구현: `maestro/core/icons.js`가 Lucide path를 *필요한 것만* 정의하고 `window.AAP_ICON.svg(name)` 헬퍼로 emit.
...
### 4.3 토큰 스케일 (간격·라운드·폰트 — 매직넘버 수렴)
> 부채 감사(`aap_design_debt_audit_v0_1.md` D3·D4): 라운드 10종·폰트 14종 난립. 아래 스케일로 수렴, 신규 UI는 이 토큰만.
- **간격**: `--sp-1:4 · --sp-2:6 · --sp-3:9 · --sp-4:11 · --sp-5:14 · --sp-6:18px`. 패딩·갭은 이 단계만.
- **라운드**: `--r-chip:7 · --r-node:9 · --r-card:12 · --r-panel:15 · --r-pill:999px`.
- **폰트**: `--fs-micro:9(800·UPPER) · --fs-sub:11 · --fs-body:13 · --fs-h:15(800) · --fs-hero:17px`.
- 기존 매직넘버는 감사 펀치리스트대로 **점진 치환**(다크 캔버스·soft 색은 정당, 보존).

## 5. 핵심 컴포넌트 패턴

### 5.1 Top bar (54px, 흰 배경)
`로고(teal 그라데 사각) + 제품명(AAP Maestro) + Pack 칩(현재 도메인) … [구성/실행/관리 토글] [Run 버튼(--ink, 실행 중 stop=red)]`
...
### 5.6 HITL 결정 = 인라인 요약 + on-demand 모달 (v0.9 정정)
> 외부 지시서(운영 콘솔 재설계) 반영. 기존 "HITL=모달" → **결정 요약은 인라인, 조정은 모달**로 정정.
- **인라인(기본·항상 보임)**: 게이트 도착 시 모달이 화면을 **자동으로 덮지 않는다**(`STATE.baseOnly=true`). 본문에 **결정 요약 카드**(브리핑 `#runBrief` + 콘솔 인라인 판정/랭킹 + 상단 "여기서 결정이 필요해요·결정하기" 큐). 색=amber(§5.5). 사람이 *무엇을 왜* 정해야 하는지 흐름 안에서 읽힌다.
- **모달(on-demand)**: "결정하기/기준 조정" 클릭 시에만(`openGate`) — **기준 조정(임계 슬라이더·가중·통과 미리보기)·상세 근거·비교표·긴 리포트·산출물**. Notion식 차분함(흰 표면·여백·상단 back teal·닫기 X).
- **결정 후 피드백**: `decide()`가 토스트(도메인 라벨)+트레이스(HITL Decision Log)+다음 단계 자동 진행. 적용 기준·변경 결과가 다음 단계 브리핑/콘솔에 반영.
- 구현: `core.js` `revealOps`(게이트 baseOnly=true)·`renderRunBrief`·`renderRunAction`(결정하기→`openGate`)·`AAP_CORE.openGate/atGate`. 수제 surface 게이트 버튼=게이트면 모달, 아니면 이동.
...
### 5.9.1 색 의미축 ↔ 형태축 매핑 (4축 충돌 방지 · v0.22 명문화)
> maestro 3차 감사: 5색(teal·violet·cyan·blue·amber)이 **여러 의미축에 동시 재사용**된다 — ① 컴포넌트 타입(§5.9) ② 객체 kind(§6.6.5: entity/event/document/master) ③ pack 식별(인박스 유형 칩 §6.5.2) ④ provenance(실행 추적: 원천/결정론/LLM/게이트 §6.6.2). 같은 violet이 *Module·문서 객체·VOC 팩·LLM 판단*을 동시에 의미할 수 있어 **색만으로는 어느 축인지 구분 불가**. → 색 토큰은 그대로 유지하되, **축마다 형태(shape)를 고정**해 충돌을 막는다.

| 의미축 | 형태 신호 (고정) | 적용 예 |
|---|---|---|
| 컴포넌트 타입 | **상단 3px 보더** + 타입 배지 | 구성 카드 §5.4 |
| 객체 kind | **좌측 보더**(또는 노드 형태) | 온톨로지 객체 카드 §6.6.5 |
| provenance | **원형 dot** + 텍스트 배지 | 실행 추적 §6.6.2 |
| pack 식별 | **칩(pill)** | 인박스 유형 칩 §6.5.2 |

- 한 화면에 의미축이 둘 이상 겹치면 색이 같아도 **형태로 구분**된다(상단보더 ≠ 좌측보더 ≠ dot ≠ 칩). 새 화면은 이 매핑을 지킨다.
- 대개 현행 코드가 이미 준수 — 본 절은 **표준화로 회귀 차단**(형태축을 임의로 바꿔 색 충돌을 재유발 ✕).
...
### 5.11 빈 상태 · 로딩 · 에러 (상태 패턴)
운영 콘솔은 실제 업무가 비거나·처리 중이거나·막히는 경우가 잦다 — 일관 패턴으로.
- **빈 상태**: 중앙 옅은 안내 1줄(faint) + Lucide muted 아이콘 + **1차 액션**(예: 인박스 빈칸 → "＋ 새 업무 요청"). 죄책감 ✕, 다음 행동 제시. (현 `.ibx-empty` 패턴 표준화)
- **로딩/처리 중**: `spin`(§6.9) + "AAP가 ~중" 1줄(업무 언어). 카드 자리엔 옅은 스켈레톤 박스.
- **에러·차단**: red(§3) 좌측 보더 + 사유 1줄 + 복구 액션. 게이트 0개 차단(§6.6.3)이 대표. **raw 스택/JSON ✕**(업무 언어로).
- **부분 결과**: "준비 전"·"대기" faint 라벨(근거 레일 op `대기` 패턴).

### 5.12 접근성 (a11y · 권장 표준)
- **키보드**: 모든 클릭 요소 `tabindex` + Enter/Space 작동(노드·탭·카드·토글·접기). 포커스 가시 = teal ring `:focus-visible`.
- **대비**: 본문 ≥ 4.5:1. **다크 캔버스 indigo·faint 텍스트 대비 점검**(저대비 주의 — 부채 감사 D2 연계).
- **모션**: `@media (prefers-reduced-motion: reduce)` 시 §6.9 트랜지션·무한 모션 축소.
- **의미 보조**: 아이콘 단독 버튼에 `aria-label`/`data-tip`. **상태색에 텍스트/배지 병행**(색각 보조 — 색만으로 의미 전달 ✕).
...
*(`05_범용플랫폼/maestro/` 구현 = 표준. 골격은 보존, 시각만 정교화한 v0.2 반영.)*
...
*(코어 `renderOntology` 구현 = 표준. **객체 카드(§6.6.1)가 기본**, 그래프는 관계 조망용 토글 — v0.22 정정: 카드가 가독성 우위라 기본으로.)*
...
- **토글**: 상단 `카드 / 그래프` 토글. **기본 = 객체 카드**(가독성), 그래프는 관계 조망용.
...
*(전 뷰 일관 · run 뷰 패턴 기준. 구현은 maestro 세션이 일괄 — 본 절은 시각/UX 표준.)*

### 6.7.1 1뷰포트 · 스크롤 정책 (surface별 · v0.22 정정)
> maestro 3차 감사: "전 뷰 1뷰포트"는 모니터링성 관리 뷰엔 비현실적(KPI·매트릭스·로그가 길다) → surface 성격별로 분리.
- **운영(인박스·실행)·스튜디오(빌더·온톨로지) = 1뷰포트 맞춤**(run 뷰 패턴). 페이지 전체 세로 스크롤 ✕ → 콘텐츠 길면 **해당 패널 내부만** 스크롤. `body{overflow:hidden}` 기반.
- **관리 뷰(거버넌스·실행 추적·자산 카탈로그)는 내부 세로 스크롤 허용.** KPI·매트릭스·로그·요약이 길어지는 모니터링 surface는 내부 스크롤이 정석(Datadog/Grafana §1). 단 전체 페이지 점프식이 아니라 **콘텐츠 영역 내부 스크롤**(스크롤바 비표시 §6.7.2 + 하단 fade 힌트)로.
...
- 온톨로지는 **객체 카드 기본**(§6.6.1) — 그래프(§6.6.5)는 관계 조망용 토글.
...
## 6.9 모션 표준 (상태 전이 전용 · 카탈로그)

> 원칙(§2-5) = **모션은 상태 전이에만**. 장식 모션 ✕. 아래가 플랫폼 전체 모션의 전부이며, 새 모션은 이 카탈로그에 맞춰 추가한다.

| 종류 | 키프레임/방식 | 용도 | 지속 |
|---|---|---|---|
| **실행 중** | `spin`(스피너) · `pulse`/`agpulse`(노드 글로우) · `blink`(LIVE 점) | "지금 처리 중" | 0.7~1.25s, 무한 |
| **등장** | `fade` · `slidein`(아티팩트) · `recrIn`(보드 카드 **stagger** 순차) | 새 요소 진입 | 0.3~0.4s, 1회 |
| **진행** | 진행바·점수 바 `width` 트랜지션 · count-up(KPI·점수) | 값 변화 | 0.3~0.6s |
| **흐름** | `flowping`(연결선 데이터 점 `.flow-dot`) | 데이터 이동(다크 맵) | 1.1~1.25s |
| **전환** | 패널 접기 `.2s` · 스포트라이트 이동 `.3s cubic-bezier` · 탭/뷰 전환 fade | 레이아웃·포커스 이동 | 0.2~0.3s |
| **커서**(시연) | `guide-cursor` 이동 + 은은한 펄스 | 가이드 투어 안내 | — |

- **ease**: 등장·전환=ease-out / 스포트라이트=cubic-bezier. 동시 다발 모션 ✕(한 번에 하나의 상태 전이가 읽히게).
- **접근성**: `prefers-reduced-motion` 존중(가능하면 트랜지션 축소) — 권장.
- **금칙**: 의미 없는 반복 애니메이션 · 호버마다 과한 움직임 · 로딩 외 무한 모션 남발 ✕.
...
- [ ] 모션은 §6.9 카탈로그 내(상태 전이 전용). 동시 다발·장식 모션 0.
- [ ] 메인은 가볍게, 상세는 모달.
- [ ] 오버플로우·줄바꿈 깨짐 없다(긴 한글 라벨 nowrap 확인).
- [ ] 밀도 규칙(§4.1): 주 액션 ≤5, 화면 컴포넌트 ≤10, 여백 30–40%, 필터 좌측.
- [ ] **1뷰포트(§6.7)**: 페이지 전체 스크롤 ✕ → 패널 내부만. **스크롤바 비표시 + 하단 fade 힌트**.
- [ ] **패널 접기(§6.7.3)**: 좌 nav·우 패널 접기 동작·상태 영속. 접어도 demo nav 클릭 가능.
- [ ] **온톨로지(§6.6.5)**: **객체 카드 기본** + 그래프 토글(객체=노드·관계=엣지·Action 자동green/사람확인amber 배지). ontology 없으면 폴백.
- [ ] **색축↔형태축(§5.9.1)**: 의미축(타입·kind·provenance·pack)이 겹치면 색이 같아도 형태(상단보더/좌측보더/dot/칩)로 구분.
- [ ] **스크롤 정책(§6.7.1)**: 운영·스튜디오=1뷰포트 / 관리 뷰(거버넌스·실행추적·자산)=내부 세로 스크롤 허용.
- [ ] **결정론/LLM(§6.6.2)**: 블록·op·근거에 결정론(slate)/LLM(violet) 배지 일관.
- [ ] **케이스 한정(§6.8)**: 케이스 델타는 ⚑ amber 배지, 팩 기본은 중립. 승격은 주의 톤.
- [ ] **아이콘(§4.2)**: Lucide 인라인·`currentColor`만(SVG hex fill ✕), 이모지(플랫폼 chrome) 0.
...
- 자기설명 나레이션 캡션(화면이 자기 기능을 설명하는 문구) ✕ — 구조·배지로 읽히게 (§6.6.1)
- §6.9 카탈로그 밖 장식 모션 ✕

---

## 9. 컴포넌트 인벤토리 (재사용 클래스 카탈로그 · 디자인 SSOT)
> 표면별 핵심 클래스. 새 화면은 **신규 클래스 발명 전 여기서 재사용**(부채 D 방지).
- **셸**: `.topbar · .shell · .gnav(.gnav-i/.gnav-grp) · .views/.view · .scroll/.wrapc · .nav-toggle`
- **인박스**: `.inbox/.ibx-row · .ty-filter/.ty-chip · .ibx-bar · .ibx-empty(빈 상태)`
- **실행(run)**: `.run-main · .run-surface(.con-head/.con-body) · .run-evid(.evid-row/.ev-kind/.ev-more) · .cmodal · .seq/.snode · .panel-fold/.panel-handle`
- **스튜디오**: `.dom-shell/.dom-tabsbar/.dom-pane · .catalog/.cat-card(.cat-dep) · .onto-*(그래프) · .wf-builder(.wfb-palette/.wfb-canvas/.wfb-run) · .plg-*(노드그래프) · .wfp-*/.wfr-*`
- **자산·로그·운영**: `.reg/.ag · .asset-scope/.asset-filter · .log-kpis/.log-kpi · .trace · .ops-*/.eval-*/.rbac-*/.rb-*/.ao-* · .gov-strip`
- **공통**: `.comp/.ct2(5타입 칩) · .ty-badge · .s-*(상태배지) · .cp-btn(.primary/.ghost) · .case-tune(⚑) · .demo-card`
- **5타입 색 클래스**: `tA·tM·tS·tC·tP`(compose `.cls`) / `tyA~tyP`(components `.ty`) — §5.9·X1 계약.
...
*v0.8 — §4.3 토큰 스케일(간격·라운드·폰트, 매직넘버 수렴) + §5.11 빈/로딩/에러 상태 패턴 + §5.12 접근성(a11y) 표준 + §9 컴포넌트 인벤토리(재사용 클래스 카탈로그). 부채 감사 `aap_design_debt_audit_v0_1.md` 동반(스크롤바 미숨김 D1·하드코딩 hex D2·라운드 D3·폰트 D4 펀치리스트). 문서만.*
*v0.7 — §6.9 모션 표준(상태 전이 전용 카탈로그: 실행/등장/진행/흐름/전환/커서) + §7 체크리스트에 1뷰포트·접기·온톨로지 그래프·결정론/LLM·케이스 한정·아이콘 항목 추가 + §8 금칙(자기설명 캡션·카탈로그 밖 모션). 디자인 가이드 표준 절 완비. 시각만·신규 hex 0.*
...
*v0.9 — §2-6·§5.6 정정: **HITL = 인라인 결정 요약 + on-demand 모달**(기존 "HITL=모달"에서 전환). 외부 지시서(운영 콘솔 재설계) 반영 — 게이트 도착 시 덮는 모달 자동 표시 ✕(baseOnly=true), 본문 인라인 결정 카드(브리핑+콘솔+'결정하기' 큐), 기준 조정·상세만 모달. 구현=`05_범용플랫폼/app/`(revealOps·renderRunBrief·openGate/atGate). 신규 hex 0.*
*v0.22 — maestro 3차 감사 표준 정정 3건: **§5.9.1 색 의미축↔형태축 매핑**(5색이 4축[타입·kind·provenance·pack]에 겹쳐도 형태[상단보더/좌측보더/dot/칩]로 구분, 회귀 차단) · **§6.7.1 스크롤 정책 surface별 분리**(운영·스튜디오=1뷰포트 / 관리 뷰[거버넌스·실행추적·자산]=내부 세로 스크롤 허용 — Datadog 톤) · **§6.6.5 온톨로지 객체 카드 기본**(그래프=관계 조망 토글, 가독성 우위). + 라이브 기준선 경로 `app/`→`maestro/`(AAP Maestro 정본 승격)·제품명 AAP Platform→AAP Maestro·§7 체크리스트 2항 추가 반영. 문서만·신규 hex 0.*
