...
> **주의**: 다크 캔버스 안에서는 실행=**indigo**(오케스트레이션 원본 유지, 글로우 가독성). 라이트 표면의 teal 주색과 역할이 다르다 — *다크 맵=기계 작동(indigo glow), 라이트 표면=업무 주체(teal)*. 둘을 섞지 말 것.
**모션**: `@keyframes agpulse`(실행 중 노드 글로우), `fade`(등장). 1.15s 안팎, 과하지 않게.
