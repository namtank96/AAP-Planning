...
### LLM 분해 프록시 배선 굳히기 — maestro 정본 기준 문서 현행화 (코드 0) (260629, 유저 지시) 〔표준·정의〕
> `_dev_llm_proxy/`만(maestro 코드 미접촉 — 배선은 app_v2 상속분으로 **이미 완성**). 점검 결과: 클라이언트 체인(`maestro/core/viz_bridge.js analyze`=결정론 detViz 즉시→`llm_breakdown.js run()` 라이브 업그레이드·`.catch` 폴백 / index.html 3스크립트 포함 / 새 업무 요청·AI FDE가 `AAP_VIZ_BRIDGE.analyze` 호출)·server.js(포트 8787=클라 BASE 일치·`/health`·`/breakdown`·SCHEMA가 viz_decomp 소비형 work/compose/gates/ontology/lineage 정합·touch 배열→객체 변환·`output_config.format` json_schema·`claude-sonnet-4-6`/`haiku` 허용·프롬프트 캐싱) **전부 현행 API로 정확**. 코드 수정 불요. **문서만 현행화**: INTEGRATION.md=구 "app/ owner-세션 패치 목록"(무효) → "maestro 배선 완료·프록시만 켜면 라이브" 가이드로 전면 재작성, README.md app/→maestro·폴백을 detViz로 정정, test_live.html 코멘트 정정(.env.example는 이미 정확=미변경). selective-API 원칙 명문화(API는 분해 생성 순간만·매칭/게이트/런타임/계보=결정론). **라이브 켜기=프록시 `npm i`+키+`npm start`(8787)면 maestro 분해 자동 라이브, 끄면 결정론**(file:// 항상 작동). 라이브 E2E 스모크(실 API 1회·비용)는 유저 옵트인 대기.
