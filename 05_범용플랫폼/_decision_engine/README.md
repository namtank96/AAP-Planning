...
### 4개 도메인 전수 게이트 통과 + expectedOutcome 정밀화 (260623)
| 도메인 | 게이트 | 라운드 | 독립 재검증(evaluate.js, expectedOutcome 기준) | drift |
|---|---|---|---|---|
| 계약 A | PASS | 1 | 10/10 | 0 |
| 계약 B | PASS | 1 | 9/9 | 0 |
| 구매·조달 | PASS | **1** (정밀화 전 3) | 11/11 | 0 |
| 경비·지출 | PASS | 1 | 11/11 | 0 |
- 게이트가 도메인별 고유 결함 자동 해소: 구매=config 임계 슬롯 6종 drift+임계값 seed 역산 / 경비=th_amount_tolerance 리터럴 치환 / 계약=toxicHighCount 슬롯+보완 룰 R6B.
- **✅ 정밀화 완료**: 게이트 기대-outcome 도출이 seed 라벨 정규식(`_outcomeOf`)이라 "자동승인 **차단**" 부분문자열 오매칭 → **`annotate` 단계가 seed에 `expectedOutcome` enum 명시**, 게이트가 그 필드 사용(정규식 의존 제거). 효과: **구매 3R→1R**(헛 라운드 제거), 계약 B "담당→대표 정상결재"가 부정확한 LEGAL→정확한 AUTO로 정렬.

## 다음 (이 트랙·app/ 무관)
- **드롭인(`dropin/`)**: 3팩(`pack_contract_A`·`pack_procurement`·`pack_expense`.decision.json) = **하니스 자동생성 + 게이트 통과 + expectedOutcome 명시**(수기 0, 독립 재검증 전수 ✅). app/ 통합 = `dropin/INTEGRATION.md`(≈3줄+헬퍼).
- (선택) 추가 도메인 / N2 소비 방식.
