...
1. **6단계 파이프라인**: 접수→슬롯 추출→결정론 룰 1차판정→HITL 게이트→route 3분기→결재선→writeback. *같은 단계 유형 서열(아키타입)을 공유 — 단계 수는 도메인별 가변.*
...
### 10.5 결론
> **"범용 시나리오 공장" 입증됨.** 구매·경비가 계약과 **동일 아키타입**(단계 유형 서열)·decide 2계층·route 3분기 서열·gap↔게이트·임계 패밀리·writeback·**같은 엔진**을 공유하고(단계 *수*는 가변 — 계약A 13 vs 구매·경비 7), 도메인 차이는 knowledge·슬롯 소스에만 국한. 같은 하니스 골격에 knowledge·슬롯만 갈아끼우면 도메인이 무수정 1패스로 찍혀 나온다. → 하니스 트랙(가설 검증)은 **여기서 일단락**: 신뢰 가능(N1b) + 범용(N4) + 고객 변수 조건화(이전) 3박자 확보.
