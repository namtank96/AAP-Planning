...
function dcVerdictCard(C){
  ...
  return `<div class="ct-vd ${m.cls}${reflow?' reflow':''}">
    <div class="ct-vd-top"><span class="ct-vd-ic">${_ICO(m.ic)}</span>
      <div><div class="ct-vd-h">${m.ko}<span class="ct-vd-tag">verdict</span></div><div class="ct-vd-sub">${dcText(v.label,'verdict.label')||m.sub}</div></div></div>
...
function dcSlotPanel(C){
  ...
  const cell=(key)=>{ const s=slotIdx[key]; if(!s)return ''; const val=dcSlotVal(C,key);
    ...
    return `<div class="ct-slot${cls}"><div class="ct-slot-k">${s.label}<span class="ct-slot-src">${DC_EXKO[s.extract]||s.extract||''}</span></div>
      <div class="ct-slot-v">${val==null?'<span class="ct-slot-na">미추출</span>':val}</div></div>`; };
  const prim=dcPrimary(C); const primSet=new Set(prim);
  const primHtml=prim.map(cell).join('');
  const rest=dcSlots().filter(s=>!primSet.has(s.key)).map(s=>cell(s.key)).join('');
  return `<div class="ct-wh"><span class="op-t">케이스 슬롯</span><span class="op-c">caseModel · 값·출처·추출방식</span>
      <button class="ct-slot-more" data-dcslots>${open?'접기':'전체 슬롯'}</button></div>
    <div class="ct-slots">${primHtml}</div>
...
function dcSteer(C){
  ...
  return `<div class="op-steer">${eds.map(grp).join('')}
...
function dcAside(C){
  ...
  return `${lks?`<div class="op-sh">참조 룩업 <span>지식·시맨틱 L4</span></div><div class="ct-lks">${lks}</div>`:''}
    <div class="op-sh">AAP가 한 일 · 근거 <span>보조</span></div>
    <div class="op-flow">
      ${evRows||dcFstep('check','케이스 슬롯 채움','문서·커넥터·규칙 추출')}
      ${dcFstep('arrow-right','결정 룰 판정', v?(m.ko+' · '+v.ruleId):'대기','v')}
    </div>`;
}
